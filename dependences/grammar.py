...
class Grammar:
    def __init__(self, productions=None, start=None, epsilon='λ', eof='$'):
    ...
    def first(self, x):
        ...
        2- If there exists a production X -> λ, FIRST(X) = {λ}
    ...
    def follow(self, nonterminal, previous=MemoHelper()):
        ...
        1.  For each production X -> aAb, put FIRST(b) − {λ} in FOLLOW(A)
        2.a For each production X -> aAb, if λ is in FIRST(b) then put FOLLOW(X) into FOLLOW(A)
    ...
    def parsing_table(self, is_clean=True):
        ...
        """
        from functions import remove_left_recursion, remove_left_factoring  # To avoid cyclic import

        equiv = self if is_clean else remove_left_recursion(
            remove_left_factoring(copy(self)))
